...
const SoatPage = ({ showConfirm, softphone }) => {
  ...
  const gestionEntries = useMemo(() => clientes.flatMap(c => {
  ...
  }).sort((a, b) => parseHistFecha(b.fecha) - parseHistFecha(a.fecha)), [clientes]);
  ...
  return (
    <div>
      ...
      <div style={{ ...S.tableWrap, overflowX: "auto" }}>
        <div style={{ minWidth: filtroFase === "ilocalizable" ? 920 : 860 }}>
          <div style={{ ...S.tableHead, display: "grid", gridTemplateColumns: filtroFase === "ilocalizable" ? "36px 2fr 0.8fr 0.8fr 1.3fr 1fr 1.2fr 52px 90px" : "36px 2fr 0.8fr 0.8fr 1.3fr 1fr 1.2fr 90px" }}>
            <span>#</span><span>Cliente</span><span>F. Base</span><span>F. Vencimiento</span><span>Fase</span><span>Agente</span><span>Próxima acción</span>
            {filtroFase === "ilocalizable" && (
          ...
          {filtrados.length === 0 ? (
            ...
            return (
              <div key={c.id}
                style={{ ...S.tableRow, display: "grid", gridTemplateColumns: filtroFase === "ilocalizable" ? "36px 2fr 0.8fr 0.8fr 1.3fr 1fr 1.2fr 52px 90px" : "36px 2fr 0.8fr 0.8fr 1.3fr 1fr 1.2fr 90px" }}
                ...
                {filtroFase === "ilocalizable" && (
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "center" }}>
                    <button
                      onClick={() => updateC(c.id, "whatsapp", !c.whatsapp)}
                      title={c.whatsapp ? "WhatsApp enviado ✓" : "Marcar como enviado por WhatsApp"}
                      style={{ width: 30, height: 30, borderRadius: "50%", border: c.whatsapp ? "none" : "2px solid #d1d5db", background: c.whatsapp ? "#25D366" : "#fff", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", transition: "all 0.15s", flexShrink: 0 }}>
                      {c.whatsapp
                        ? <svg viewBox="0 0 24 24" width="15" height="15" fill="#fff"><path d="M20.285 2l-11.285 11.567-5.286-5.011-3.714 3.716 9 8.728 15-15.285z"/></svg>
      ...
      {showGestion && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(7,29,71,0.55)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 250, padding: 16 }} onClick={() => setShowGestion(false)}>
          <div style={{ background: "#fff", borderRadius: 18, width: "100%", maxWidth: 920, maxHeight: "92vh", overflowY: "auto", boxShadow: "0 20px 60px rgba(26,86,219,0.25)", padding: "28px 32px" }} onClick={e => e.stopPropagation()}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
              <div>
                <div style={{ fontWeight: 800, fontSize: 20, color: BLUE.text }}>Gestión</div>
                <div style={{ fontSize: 13, color: "#6b87b0", marginTop: 3 }}>
                  {gestionEntries.length} gestión{gestionEntries.length !== 1 ? "es" : ""} registrada{gestionEntries.length !== 1 ? "s" : ""}
                </div>
              </div>
              <button onClick={() => setShowGestion(false)} style={{ background: "none", border: "none", fontSize: 24, cursor: "pointer", color: "#aaa" }}>×</button>
            </div>

            {gestionEntries.length === 0 ? (
              <div style={{ textAlign: "center", color: "#aaa", padding: 48, fontSize: 14 }}>Sin gestiones registradas.</div>
            ) : (
              <div style={{ ...S.tableWrap, overflowX: "auto" }}>
                <div style={{ minWidth: 780 }}>
                  <div style={{ ...S.tableHead, gridTemplateColumns: "90px 1.6fr 1.6fr 1fr 2fr" }}>
                    <span>Fecha</span><span>Cliente</span><span>Cambio</span><span>Agente</span><span>Detalle</span>
                  </div>
                  {gestionEntries.map((h, i) => {
                    const f = FM_SOAT[h.resultado];
                    ...
                    return (
                      <div key={i}
                        ...
                        style={{ ...S.tableRow, gridTemplateColumns: "90px 1.6fr 1.6fr 1fr 2fr", cursor: "pointer", alignItems: "flex-start" }}
                        onMouseEnter={e => e.currentTarget.style.background = BLUE.light}
                        onMouseLeave={e => e.currentTarget.style.background = ""}>
                        <div style={{ fontSize: 12, color: "#555" }}>{h.fecha}</div>
                        <div>
                          <div style={{ fontWeight: 700, fontSize: 13, color: BLUE.primary }}>{h._nombre || "—"}</div>
                          <div style={{ fontSize: 11, color: "#999" }}>{h._telefono}{h._placa ? ` · ${h._placa}` : ""}</div>
                        </div>
                        <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap", opacity: cambio ? 1 : 0.55 }}>
                          <span style={{ fontSize: 11, fontWeight: 700, color: fDesde?.text, background: fDesde?.bg, padding: "2px 8px", borderRadius: 20, whiteSpace: "nowrap" }}>{fDesde?.label || h._desde}</span>
                          <span style={{ fontSize: 12, color: "#999" }}>→</span>
                          <span style={{ fontSize: 11.5, fontWeight: 700, color: f?.text, background: f?.bg, padding: "2px 9px", borderRadius: 20, whiteSpace: "nowrap" }}>{f?.label || h.resultado}</span>
                        </div>
                        <div style={{ fontSize: 12, color: "#888" }}>{h.agente}</div>
                        <div style={{ fontSize: 12, color: "#666" }}>
                          {(h.motivo || h.motivoIloc) && <div>Motivo: {h.motivo || h.motivoIloc}</div>}
                          {h.proximaAccion && <div>Acción: {h.proximaAccion}{h.fechaProxima ? ` · ${h.fechaProxima}` : ""}</div>}
                          {h.nota && <div style={{ fontStyle: "italic", color: "#444" }}>"{h.nota}"</div>}
                        </div>
                      </div>
                    );
                  })}
